...
class QueueServiceIntegrationTest {
	...
	void clearRedis() {
		...
		Set<String> activeScheduleKeys = stringRedisTemplate.keys("active:schedules");
		if(activeScheduleKeys != null && !activeScheduleKeys.isEmpty()) {
			stringRedisTemplate.delete(activeScheduleKeys);
		}
	}
	...
	@Test
	void scheduledCleanup_호출시_heartbeat없는_user는_active_wait에서_제거된다() {
		Long scheduleNo = 1L;
		...
		stringRedisTemplate.opsForSet().add(activeSchedulesKey, String.valueOf(scheduleNo));

		queueService.scheduledCleanup();

		Boolean activeContainsStaleUser =

				stringRedisTemplate.opsForSet().isMember(activeKey, staleActiveUser);
		Double waitScore =

				stringRedisTemplate.opsForZSet().score(waitKey, staleWaitUser);
