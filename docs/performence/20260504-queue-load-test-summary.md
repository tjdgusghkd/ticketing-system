# 부하 테스트 리포트: 대기열(Queue) 성능 개선 및 한계 분석
  ...
  - **테스트 목적:** 대기열 구조 리팩토링 이후 응답 성능, 실패율, CPU 사용률 변화를 	확인하고 단일 인스턴스 기준 임계 구간을 파악
  ...
  - 톰캣 thread / connection 설정 실험
  - HikariCP 설정 실험
  - Redis Lettuce pool 설정 실험
  ...
  - 특히 `checkQueue()` cleanup 제거와
  `queue-status.lua` 조회 전용 분리는 실제 성능 개선에 가장 큰 영향을 준 것으로 판단된다.
  ...
  ## 7. 설정 튜닝 결과
  - 톰캣 설정(`threads`, `accept-count`, `max-connections`) 조정은 기존 기준보다 뚜렷한 개선을 만들지 못했다.
  - HikariCP 및 Redis Lettuce pool 설정 조정 역시 queue 흐름 기준 유의미한 개선이 없었다.
  - 따라서 queue 영역에서는 설정 튜닝보다 코드 리팩토링 효과가 더 크게 확인되었다.

  ## 8. 결론
  - 대기열 구조 리팩토링은 실제 성능 개선 효과가 있었다.
  - `queue-status`는 단일 로컬 인스턴스 기준 `700 VUs`까지는 안정적이었다.
  - `1000 VUs`는 경계 구간, `1500 ~ 2000 VUs`는 한계 구간으로 판단된다.
  - 현재 단계에서는 Queue 코드 리팩토링은 1차 완료된 상태이며, 추가 개선은 시스템 튜닝보다 배포 환경 재측정 및 다중 인스턴스 확장 검토가 더 적절하다.

  ## 9. 향후 계획
  - 배포 환경에서 queue/seat 흐름 재측정
  - 필요 시 `stages` 기반 점진 증가 테스트 수행
  - 로드 밸런싱 및 다중 인스턴스 환경에서 재확인